...
public:
	static constexpr const char* applicationKey = "texasgamer.Zephyr";
...
public:
	...
	bool isLoggedIn() { return userToken.isLoggedIn(); }
...
public slots:
	...
	void promptLogin();
	void logout();
	void onTokenModified(const QString& path);
...
public:
	...
	static OverlayController* createInstance(bool desktopMode, bool noSound) {
		singleton.reset(new zephyr::OverlayController(desktopMode, noSound));
		return singleton.get();
	}
