...
void installManifest(bool cleaninstall = false) {
	...
	if (QFile::exists(manifestQPath)) {
		...
		if (vr::VRApplications()->IsApplicationInstalled(zephyr::OverlayController::applicationKey)) {
			if (cleaninstall) {
				char buffer[1024];
				auto appError = vr::VRApplicationError_None;
				vr::VRApplications()->GetApplicationPropertyString(zephyr::OverlayController::applicationKey, vr::VRApplicationProperty_WorkingDirectory_String, buffer, 1024, &appError);
		...
		if (apperror != vr::VRApplicationError_None) {
			...
			auto apperror = vr::VRApplications()->SetApplicationAutoLaunch(zephyr::OverlayController::applicationKey, true);
			if (apperror != vr::VRApplicationError_None) {
...
void checkForUpdates() {
	QFile inputFile("version.txt");

	if (!inputFile.exists()) {
		LOG(INFO) << "Attempting to create version.txt...";
		QTextStream out(&inputFile);
		if (inputFile.open(QIODevice::ReadWrite | QIODevice::Truncate)) {
			out << zephyr::OverlayController::applicationVersionCode;
			inputFile.flush();
			inputFile.close();
		}
	}
	...
	if (inputFile.open(QIODevice::ReadOnly)) {
		...
		if (line.toInt() < zephyr::OverlayController::applicationVersionCode) {
			LOG(INFO) << "Zephyr was updated from " << line << " to " << zephyr::OverlayController::applicationVersionCode;
			if (inputFile.open(QIODevice::ReadWrite | QIODevice::Truncate)) {
				QTextStream out(&inputFile);
				out << zephyr::OverlayController::applicationVersionCode;
				inputFile.flush();
				...
				LOG(ERROR) << "Unable to write to version.txt!";
			}
		}
	} else {
		LOG(ERROR) << "Unable to read version.txt! Current version is " << zephyr::OverlayController::applicationVersionCode;
	}
}
...
int main(int argc, char *argv[]) {
	...
	try {
		...
		a.setOrganizationName("Zephyr");
		a.setApplicationName("Zephyr");
		a.setApplicationDisplayName(zephyr::OverlayController::applicationName);
		a.setApplicationVersion(zephyr::OverlayController::applicationVersionString);

		qInstallMessageHandler(myQtMessageHandler);

		// Load font
		QFontDatabase::addApplicationFont("res/righteous-regular.ttf");
		...
		if (!conf.get(el::Level::Global, el::ConfigurationType::Filename)) {
			logFilePath = QDir(QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)).absoluteFilePath("Zephyr.log");
		...
		zephyr::OverlayController::setAppSettings(&appSettings);
		LOG(INFO) << "Settings File: " << appSettings.fileName().toStdString();

		QQmlEngine qmlEngine;

		zephyr::OverlayController* controller = zephyr::OverlayController::createInstance(desktopMode, noSound);
		controller->Init(&qmlEngine);

		// Pass values to AboutTabController
		controller->aboutTabController.setSettingsFile(appSettings.fileName());
		controller->aboutTabController.setLogConfiguration(logconfigfile);
		...
		controller->SetWidget(qobject_cast<QQuickItem*>(quickObj), zephyr::OverlayController::applicationName, zephyr::OverlayController::applicationKey);
